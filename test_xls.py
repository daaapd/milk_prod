...
cursor = conn.cursor()
sqlSelect ="SELECT group_,sum(p1),sum(p2),sum(p3),sum(p4),sum(p5),sum(p6),sum(p7),sum(p8),sum(p9),min(c1),min(c2),min(c3),min(c4),min(c5),min(c6),min(c7),min(c8),min(c9) FROM milk group by group_"
try:
 # Execute query.
